...
def intensity_histogram_measures(regionmask: np.ndarray, intensity: np.ndarray):
    ...
    """
    # Get mode value in a way that works with newer scipy versions
    mode_result = stats.mode(intensity[regionmask], axis=None)
    ...
    feat = {
        ...
        "int_mode": float(mode_value),  # Convert to float in case it's a numpy type
